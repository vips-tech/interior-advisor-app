import { a as runtimeName, i as isContext, n as bufferRequest, o as seedContext, r as getEnv, t as defineMiddleware } from "./define-middleware-8DuseuLv.mjs";

//#region src/core/define-composite.ts
/**
...
function defineComposite(spec) {
	...
	const callable = (...args) => {
		...
		const boundary = internal.length === 0 ? handler : (req, ctx) => {
			const upstream = ctx[snapshotKey];
			const visible = { ...ctx };
			delete visible[snapshotKey];
			for (const key of internal) if (upstream && key in upstream) visible[key] = upstream[key];
			else delete visible[key];
			return handler(req, visible);
		};
		const folded = spec.build(config).reduceRight((h, entry) => entry(h), boundary);
		...
		return (req, arg, ...rest) => {
			...
			const upstream = arg;
			const snapshot = {};
			for (const key of internal) if (key in upstream) snapshot[key] = upstream[key];
			return run(req, {
				...arg,
...
* @category Composition
*/
function pipeline(entries, handler) {
	return entries.reduceRight((h, entry) => entry(h), handler);
}

//#endregion
export { bufferRequest, defineComposite, defineMiddleware, getEnv, isContext, pipeline, runtimeName, seedContext };